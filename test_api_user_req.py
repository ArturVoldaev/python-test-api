...
class TestClass:
  ...
  def generate_data(self):
    data = generate_body_for_user.UserBody(self.fake.unique.random_int(),
  ...
  def test_03_put_user(self):
    ...
    assert self.user_put_req(data).json().get('code') == 200

  def test_04_delete_user(self):
    data = self.generate_data()
    assert self.user_delete_req(data).status_code == requests.codes.ok
    assert self.user_delete_req(data).json().get('code') == 200
